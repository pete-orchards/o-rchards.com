...
@section('main')
	...
	@include('emails.components.text', ['text' => $idea_theme_post->post->user->name.'様、テーマ企画にご参加くださいまして、ありがとうございました。あなたの投稿が見事、入賞しましたのでお知らせします。'])
	@include('emails.components.image', [
		'src' => $idea_theme_result->banner ? url('/'.$idea_theme->result->banner_path()) : url('/'.$idea_theme->banner_path())
	])
