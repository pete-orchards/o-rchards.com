	...
	<div class="js-add-target">
		...
		@if($errors->$type->all() || !empty($btn_back_msg))
			...
			@foreach(old($kind) as $key => $val)
				@php
					$val['subtotal'] = $val['amount'] * $val['volume'];
					$gross = $gross + $val['subtotal'];
				@endphp
				@include('components.form.income_cost_container', [])
			@endforeach
		@else
			@include('components.form.income_cost_container', ['key' => '0'])
		@endif
		...
		<div class="text-right py-1 px-4 m-1 text-lg">
			@if($kind == 'incomes'){{'収入'}}@elseif($kind == 'costs'){{'支出'}}@endif計: <span class="js-gross">{{ $errors->$type->all() ? $gross : 0}}</span>円
		</div>
	</div>
</div>
