<div class="border-2 border-lettuce-300 rounded-xl bg-white mx-auto p-2">
	<form action="{{route('nae.store')}}" method="post" enctype="multipart/form-data" class="js-nae-form" data-target="nae">
		@csrf
		...
		<div class="my-4 js-length-count-container">
			...
			<div class="">
				<input class="bg-lettuce-50 rounded-full w-80 max-w-full p-1 border border-lettuce-300 focus:border-tomato-300 focus:outline-none js-length-count-target" id="nae/title" type="text" name="title" value="@if($errors->nae->all()){{old('title')}}@elseif(!empty($re_title)){{'Re:'.$re_title}}@endif" required>
			</div>
		...
		@include('components.form.income_cost', ['type' => 'nae', 'kind' => 'incomes'])
		@include('components.form.income_cost', ['type' => 'nae', 'kind' => 'costs'])

		<div class="my-4">
			@include('components.form.img', ['type' => 'nae', 'error' => $errors->nae])
		</div>

		@include('components.form.tag', ['id' => 'nae/tag', 'error' => $errors->nae])
		@auth
		@include('components.form.reference', ['id' => 'nae/reference', 'error' => $errors->nae])
		@endauth
